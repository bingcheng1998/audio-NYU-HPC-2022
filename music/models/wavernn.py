...
class WaveRNN(nn.Module) :
    ...
    def generate(self, seq_len) :
        ...
        with torch.no_grad() :
            ...
            # Some initial inputs
            out_coarse = torch.LongTensor([0]).to(self.device)#.cuda()
            out_fine = torch.LongTensor([0]).to(self.device)#.cuda()
    ...
    def init_hidden(self, batch_size=1) :
        return torch.zeros(batch_size, self.hidden_size).to(self.device)#.cuda()
