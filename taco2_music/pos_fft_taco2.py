...
loaderGenerator = MusicLoaderGenerator(labels, DATALOADER_WORKERS, use_mel=False, sample_rate=SAMPLE_RATE)
...
def load_checkpoint(path):
    if exists(path):
        ...
        if 'model_state_dict' in checkpoint:
            ...
            delet_keys_all = [q for q in checkpoint['model_state_dict'].keys() if check_delet(q)]
            for key in delet_keys_all:
...
def mse_total_loss(mels_tensor, org_mel, pos_mel):
    ...
    # loss1 += mse_loss(mels_tensor[:, :128, :], pos_mel[:, :128, :])*10
    # loss1 += mse_loss(mels_tensor[:, 128:256, :], pos_mel[:, 128:256, :])*5
    # loss1 += mse_loss(mels_tensor[:, 256:, :], pos_mel[:, 256:, :])*2
    org_mel_masked, pos_mel_masked = threshold_mse_loss(mels_tensor[:, 256:, :], pos_mel[:, 256:, :], 0.2)
...
def test():
    ...
    with torch.no_grad():
        for i_batch, sample_batched in enumerate(test_loader):
            ...
            org_mel, pos_mel, stop_token, _, _ = model.forward(sample_batched)
...
def train(epoch=1):
    ...
    for epoch in range(initial_epoch, epoch):
        ...
        for i_batch, sample_batched in enumerate(train_loader):
            ...
            org_mel, pos_mel, stop_token, _, embedded_inputs = model.forward(sample_batched)
            loss1 = mse_total_loss(mels_tensor, org_mel, pos_mel)
            ...
            # l1_lambda = 0.001
            # l1_norm = sum([embedded_input.abs().sum()  for embedded_input in embedded_inputs])
            # loss3 = l1_lambda * l1_norm
            
            # Step 3. Run our backward pass
            loss = loss1 + 50 * loss2 #+ loss3
            loss.backward()
            optimizer.step()
            ...
            if i_batch % (600 // BATCH_SIZE) == 0: # log about each n data
